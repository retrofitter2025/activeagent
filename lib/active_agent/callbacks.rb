...
module ActiveAgent
  module Callbacks
    ...
    module ClassMethods
      ...
      def before_generate(*filters, &block)
        set_callback(:generate, :before, *filters, &block)
      end

      # Defines a callback that will get called right after the
      # prompt's generation method is finished.
      def after_generate(*filters, &block)
        set_callback(:generate, :after, *filters, &block)
      end

      # Defines a callback that will get called around the prompt's generation method.
      def around_generate(*filters, &block)
        set_callback(:generate, :around, *filters, &block)
      end

      # Defines a callback for handling streaming responses during generation
      def on_stream(*filters, &block)
        set_callback(:stream, :before, *filters, &block)
      end
    end
